...
import {
  MAX_PHONE_NUMBER_LENGTH, addNumberToPhoneNumber, removeNumberFromPhoneNumber,
} from '../logic/helpers';
...
const DialNumber = ({number, addNumber}: DialNumberInterface) => {
  return <button className="dialNumber" onClick={() => addNumber(number)}>
    {number}
  </button>;
};
  ...
  return <>
    ...
    <div className="row mt-33 ms-48 h-52 gap-30 align-items-center">
      {isPhoneValid ?
        <>
          <div
            onClick={() => setChecked(!isChecked)} className="dialNumber check"
          >
            { isChecked ? '✔' : ''}
          </div>
    ...
    {/* submit button */}
    {isSubmitDisabled ?
      <button disabled={true} className="submit fail">
        Подтвердить номер
      </button> :
      <button onClick={finish} className="submit success">
        Подтвердить номер
      </button>
    }
  </>;
...
const Menu = ({onFinish}: MenuInterface) => {
  ...
  return <div className="menu">
    <p className="mt-72 fs-26 text-center">
    ...
    <Dial
      addNumber={(toAdd) =>
        setPhoneNumber(addNumberToPhoneNumber(phoneNumber, toAdd))
      }
